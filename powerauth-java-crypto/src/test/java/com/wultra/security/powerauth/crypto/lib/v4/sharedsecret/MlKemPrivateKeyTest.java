...
public class MlKemPrivateKeyTest {
    ...
    public void testMlKemDeterministic() throws Exception {
        ...
        byte[] seed = ByteUtils.concat(d, z);
        byte[] ssExpected = Base64.getDecoder().decode("mWdFqAwDHJLok+TkHTUCyv6KsCrXEz97xKGy4dYP5aE=");
        ...
        // Use fixed byte zeroed array random seed for KEM test
        byte[] seedBytes = new byte[32];
        SecureRandom secureRandom = SecureRandom.getInstance("SHA1PRNG");
        secureRandom.setSeed(seedBytes);
        MLKEMGenerator gen = new MLKEMGenerator(secureRandom);
        SecretWithEncapsulation swe = gen.generateEncapsulated(pub);
        ...
        assertArrayEquals(ssExpected, ssRecovered);
    }
    ...
    public void testMlKemPrivateKeyImportDeterministic() throws Exception {
        ...
        byte[] ssExpected = Base64.getDecoder().decode("/r+zACKsRWYTJM0ZAOqXQT6ETGiL+pAUZfFBfX4zsdk=");
