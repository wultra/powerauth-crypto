...
public class EciesFactory {
     ...
     * @param encryptorId Additional information for sharedInfo1 parameter using pre-defined constants.
     ...
     * @throws CryptoProviderException In case cryptography provider is incorrectly initialized.
     */
    public EciesEncryptor getEciesEncryptor(final EncryptorScope scope, final EciesEnvelopeKey envelopeKey,
        ...
        final byte[] sharedInfo2 = generateSharedInfo2(scope, applicationSecret, transportKey, eciesParameters, envelopeKey.getEphemeralKeyPublic());
        return new EciesEncryptor(envelopeKey, sharedInfo2);
    }
     ...
     * @throws CryptoProviderException In case cryptography provider is incorrectly initialized.
     */
    private EciesEncryptor getEciesEncryptor(final EncryptorScope scope, final ECPublicKey publicKey, final byte[] applicationSecret,
     ...
     * @throws CryptoProviderException In case cryptography provider is incorrectly initialized.
     */
    public EciesDecryptor getEciesDecryptor(final EncryptorScope scope, final EciesEnvelopeKey envelopeKey,
                                            final byte[] applicationSecret, final byte[] transportKey,
                                            final EciesParameters eciesParameters, final byte[] ephemeralPublicKey) throws GenericCryptoException, CryptoProviderException {
        final byte[] sharedInfo2 = generateSharedInfo2(scope, applicationSecret, transportKey, eciesParameters, ephemeralPublicKey);
        return new EciesDecryptor(envelopeKey, sharedInfo2);
    }
     ...
     * @throws CryptoProviderException In case cryptography provider is incorrectly initialized.
     */
    private EciesDecryptor getEciesDecryptor(final EncryptorScope scope, final ECPrivateKey privateKey, final byte[] applicationSecret,
     ...
     * @throws CryptoProviderException In case cryptography provider is incorrectly initialized.
     */
    private byte[] generateSharedInfo2(final EncryptorScope scope, final byte[] applicationSecret, final byte[] transportKey,
                                       final EciesParameters eciesParameters, final byte[] ephemeralPublicKey) throws GenericCryptoException, CryptoProviderException {
        byte[] sharedInfo2;
        switch (scope) {
            ...
            default -> throw new GenericCryptoException("Unsupported ECIES scope: " + scope);
        }
