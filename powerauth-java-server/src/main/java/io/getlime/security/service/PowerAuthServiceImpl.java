    ...
    public InitActivationResponse initActivation(InitActivationRequest request) throws Exception {
        ...
        try {
            ...
            for (int i = 0; i < PowerAuthConstants.ACTIVATION_GENERATE_ACTIVATION_SHORT_ID_ITERATIONS; i++) {
                ...
                ActivationRecordEntity record = powerAuthRepository.findFirstByActivationIdShortAndActivationStatusInAndTimestampCreatedAfter(
                        		tmpActivationIdShort,
                        		states, 
                        		new Date(timestamp.getTime() - PowerAuthConstants.ACTIVATION_VALIDITY_BEFORE_ACTIVE)
                        );
                // this activation short ID has a collision, reset it and find another one
                if (record == null) {
    ...
    public PrepareActivationResponse prepareActivation(PrepareActivationRequest request) throws Exception {
        try {
            ...
            ActivationRecordEntity activation = powerAuthRepository
                    .findFirstByActivationIdShortAndActivationStatusInAndTimestampCreatedAfter(
                    		activationIdShort,
                            states, 
                            new Date(timestamp.getTime() - PowerAuthConstants.ACTIVATION_VALIDITY_BEFORE_ACTIVE)
                    );
            ...
            byte[] C_serverPublicKey = powerAuthServerActivation.encryptServerPublicKey(
            		serverPublicKey,
                    devicePublicKey,
                    ephemeralPrivateKey,
                    activationOtp, 
                    activationIdShort, 
                    activationNonceServer
            );
    ...
    public VaultUnlockResponse vaultUnlock(VaultUnlockRequest request) throws Exception {
    	try {
            ...
            if (activation != null && activation.getActivationStatus() == ActivationStatus.ACTIVE) {
                ...
                long lowestValidCounter = ctr;
                for (long iterCtr = ctr; iterCtr < ctr + PowerAuthConstants.SIGNATURE_VALIDATION_LOOKAHEAD; iterCtr++) {
                    signatureValid = powerAuthServerSignature.verifySignatureForData(null, signature, signatureKeys, iterCtr);
                    if (signatureValid) {
                        // set the lowest valid counter and break at the lowest counter where signature validates
                        lowestValidCounter = iterCtr;
                        break;
